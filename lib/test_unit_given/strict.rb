module TestUnitGiven
  ...
  module Strict
    ...
    # Set up the conditions for the test, see TestUnitGiven::Simple::Given
    def Given(existing_block=nil,&block)
      @__next = :when
      call_block_param_or_block_given(existing_block,&block)
    end
    # Execute the code under test, failing if no Given was called first, see TestUnitGiven::Simple::When
    def When(existing_block=nil,&block)
      raise "No Given block?" unless @__next == :when
      @__next = :then
      call_block_param_or_block_given(existing_block,&block)
    end
    # Verify the results of the test, failing if no When was called first, see TestUnitGiven::Simple::Then
